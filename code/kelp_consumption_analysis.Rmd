...
ggplot(data=prop, aes(x=fct_reorder(treatment, -eat_pcnt), y=eat_pcnt)) +
  geom_col(aes(fill=treatment), show.legend=FALSE) +
  scale_fill_manual(values=c("#0072B2", "#FF9933", "pink", "#FF3333")) + #manually change point colors
  labs(x=expression(~italic(Corynactis)~' color morph'), y="Proportion of urchins that consumed kelp") +
  #geom_text(aes(label=c("B", "AB", "AB", "A")), hjust=-1, size=6, fontface="bold") +
  geom_text(label=c("(N=15)", "(N=19)", "(N=18)", "(N=18)"), color=c("white","black","black","black"), y=0.09, size=6, fontface="italic") +
  scale_y_continuous(limits=c(0,1),expand = c(0,0), breaks=c(0,0.2,0.4,0.6,0.8,1)) +
  coord_flip() +
...
ggsave(here::here("figures", "dissertation", "treatment by percent #failures_h.png"), width=6, height=4)
```


## In 2025 - let's re-do this analysis...
```{r}
data_chi <- data %>%
  mutate(outcome = ifelse(consumption_binary == 1, "success", "failure")) %>%
...
data_consumption <- data_consumption %>%
    mutate(percent_corrected_logit = log(percent_corrected / (1 - percent_corrected)))

lmm <- lmer(percent_corrected_logit ~ treatment + (1|tank_number), data = data_consumption)
summary(lmm)

ggplot(data_consumption, aes(x = fitted(lmm), y = residuals(lmm))) +
  geom_point(alpha = 0.7) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "red") +
  labs(x = "Fitted Values", y = "Residuals") +
  theme_minimal()

ggplot(data=data_consumption, aes(x=treatment, y=percent_corrected)) +
  geom_boxplot(aes(fill=treatment), show.legend=FALSE, outliers=FALSE) +
  geom_jitter(width=0.1) +
  scale_fill_manual(values=c("#0072B2", "#FF9933", "pink", "#FF3333")) + #manually change point colors
  labs(x="text", y="Proportion of kelp consumed") +
  scale_y_continuous(limits=c(0,1),expand = c(0,0), breaks=c(0,0.2,0.4,0.6,0.8,1),
...
#ggsave(here::here("figures", "dissertation", "prop_consumped.png"), width=6, height=4)
```
