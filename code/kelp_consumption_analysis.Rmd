...
prop <- prop %>%
  ...
  mutate(no_eat_pcnt = no_eat/n)

ggplot(data=prop, aes(x=treatment, y=no_eat_pcnt)) +
  ...
  labs(x="Treatment", y="% urchins that did not consume kelp") +
  theme_classic()

ggsave(here::here("figures", "treatment by percent failures.png"), width=4, height=3)
```

Need to run post hoc tests to add "A,B" on top of the columns