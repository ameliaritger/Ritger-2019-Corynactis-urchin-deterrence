...
```{r, echo=FALSE}
ggplot(cross, aes(x=corynactis_binary, y=log10(time_to_cross_cory_first_min))) +
  geom_boxplot() +
  labs(x="Corynactis presence", y="Time to cross Corynactis tile (log(min))")
```
...
qqPlot(Y)
```
# __There are many zeros, will need to correct for this or account for it in the analysis__
# __And look at urchin size effect here still ^^__
...
- #### Urchins always crossed tile when it didn’t have Corynactis (but it wasn’t statistically significant)
- #### 37.5% of the time, urchins were never deterred by tiles with Corynactis
- #### There was no difference between color morphs affecting the "outcome"
